...
X_train = data.iloc[:split_index].drop("Dataset", axis=1)
y_train = data.iloc[:split_index]["Dataset"]
X_test = data.iloc[split_index:].drop("Dataset", axis=1)
y_test = data.iloc[split_index:]["Dataset"]
...
)  # Use 'binary' if it's a binary classification
...
alamine_aminotransferase = st.number_input(
    "Alamine Aminotransferase", min_value=0, value=20
)
aspartate_aminotransferase = st.number_input(
    "Aspartate Aminotransferase", min_value=0, value=30
)
...
albumin_and_globulin_ratio = st.number_input(
    "Albumin and Globulin Ratio", min_value=0.0, value=1.0
)
